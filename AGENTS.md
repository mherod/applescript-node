...
**DON'T** use multi-line TypeScript template literals with `${...}` spanning multiple lines in README code examples. The `Codex-review` CI workflow runs `gh pr diff` and the bash parser tries to expand `${variableName}` as a shell variable, failing with `Bad substitution: variableName`. This affects any `${expr\n.method()}` pattern where the closing `}` is on a different line than the opening `${`.
